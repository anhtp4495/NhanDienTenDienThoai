...
import codecs
from NERalphabet import Alphabet
import numpy as np
import pickle
import keras
...
def padding(input_sequence, max_len):
    pad_sequence = keras.preprocessing.sequence.pad_sequences(input_sequence, maxlen=max_len, padding='post', value=0)
    return pad_sequence
...
def construct_tensor_word(word_sentences, unknown_embedd, embedd_words, embedd_vectors, embedd_dim, max_length):
    print("%s length: %s..." % ('construct_tensor_word', len(word_sentences)))
...
def create_wordtest(input_file):
    with codecs.open(input_file, 'r', 'utf-8') as f:
        word_list = []
        words = []
        num_sent = 0
...
def predict_to_file(predicts, tests, alphabet_tag, output_file, input_file):
    word_test=create_wordtest (input_file)
    with codecs.open(output_file, 'w', 'utf-8') as f:
        for i in range(len(tests)):
            for j in range(len(tests[i])):
                ...
                f.write(word_test[i][j] + ' ' + predict + ' ' + test + '\n')
            f.write('\n')
'''
...
if __name__ == "__main__":
    create_data('words.pl', 'vectors.npy', 'data/train_sample.txt','data/dev_sample.txt', 'data/test_sample.txt')
